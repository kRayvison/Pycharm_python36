...
from CommonUtil import decorator_use_in_class
...
class PreMax(Max):
    ...
    def __init__(self,**paramDict):
        ...
        self.MAX_PREPROCESS=False  # maxcmd.txt and so on
        self.NOT_RUN_MAX=False  # notrunmax.txt
        self.NET_RENDER = False  # netrender.txt
    ...
    # @decorator_use_in_class(self.G_DEBUG_LOG)
    def RB_CONFIG(self):#4
        ...
        self.G_DEBUG_LOG.info('NOT_RUN_MAX:{}'.format(self.NOT_RUN_MAX))
    ...
    def RB_RENDER(self):#5
        """
            1.copy asset
            2.handle something(e.g. decompressing max.7z, handle ifl, handle ies)
            3.run maxscript
            4.pack
        """
        ...
        if os.path.exists(net_render_txt):
            self.NET_RENDER = True
        
        self.G_DEBUG_LOG.info('NET_RENDER:{}'.format(self.NET_RENDER))
        self.G_DEBUG_LOG.info('G_CHANNEL:{}'.format(self.G_CHANNEL))
    ...
    # @decorator_use_in_class(self.G_DEBUG_LOG)
    def copy_asset(self):
        self.G_DEBUG_LOG.info('NET_RENDER:{} == G_CHANNEL:{} == ASSET_WEB_COOLECT_BY_PATH:{}'.format(self.NET_RENDER, self.G_CHANNEL, self.ASSET_WEB_COOLECT_BY_PATH))
        if self.G_CHANNEL  == '2':  # web
            if self.NET_RENDER:  # net render
                self.copy_file_web_net_render()
            else:
                self.copy_file_web()
        else:  # client, sdk
            self.copy_file_client()
    ...
    def pack(self):
        ...
        cmd_pack = r'"{pack_exe}" a -t7z "{dest}" "{src}" -mmt -r'.format(
            pack_exe=self.G_DRIVERC_7Z,
            dest=self.G_WORK_RENDER_TASK_MAX+'.7z',
            src=self.G_WORK_RENDER_TASK_MAX+'\*',
        )
    ...
    # @decorator_use_in_class(self.G_DEBUG_LOG)
    def copy_file_web_net_render(self):        
        max_list = self.G_ASSET_JSON_DICT.get('max', [])
        self.G_DEBUG_LOG.info('max_list:{}'.format(max_list))
        
        target_path = self.G_WORK_RENDER_TASK_MAX
        for max_file in max_list:
        
            max_file = CLASS_COMMON_UTIL.bytes_to_str(max_file)
            is_exists_max_file = os.path.exists(max_file)
            
            self.G_DEBUG_LOG.info('[exists]{}:{}'.format(max_file, is_exists_max_file))
            
            if is_exists_max_file:
                cmd_max = '{fastcopy_exe} /speed=full /force_close  /no_confirm_stop /force_start "{src}" /to="{dest}"'.format(
                    fastcopy_exe=r'c:\fcopy\FastCopy.exe',
                    src=max_file.replace('/', '\\'),
                    dest=target_path.replace('/', '\\'),
                )
                # self.G_DEBUG_LOG.info('[PreMax.copyFile.cmd_max.....]'+cmd_max)
                CLASS_COMMON_UTIL.cmd(cmd_max, my_log=self.G_DEBUG_LOG)
                
                local_file_path = os.path.join(target_path, os.path.basename(max_file))
                if os.path.isfile(local_file_path):
                    if local_file_path.endswith('.max.7z'):
                        self.MAX_ZIP_LIST.append(local_file_path)
                    elif local_file_path.endswith('.ifl'):
                        self.IFL_LIST.append(local_file_path)
                    elif local_file_path.endswith('.ies'):
                        self.IES_LIST.append(local_file_path)

        
    
    # @decorator_use_in_class(self.G_DEBUG_LOG)
    def copy_file_client(self):
        asset_list = self.G_UPLOAD_JSON_DICT.get('asset', [])
        if len(asset_list) == 0:
            CLASS_COMMON_UTIL.error_exit_log(self.G_DEBUG_LOG,'[err]The asset list is empty in upload.json!')
            
        for single_asset_dict in asset_list:
            server_path_second_part = single_asset_dict.get('server').strip()
            if not server_path_second_part:
                CLASS_COMMON_UTIL.error_exit_log(self.G_DEBUG_LOG,'[err]The server path is empty in upload.json!')
                
            target_path = os.path.normpath('{}{}'.format(self.G_WORK_RENDER_TASK_MAX, os.path.dirname(server_path_second_part)))
            if self.NET_RENDER:  # net render
                if not server_path_second_part.endswith('.max') and not server_path_second_part.endswith('.max.7z'):
                    continue
                target_path = self.G_WORK_RENDER_TASK_MAX

            server_path = os.path.normpath('{}{}'.format(self.G_INPUT_USER_PATH, server_path_second_part))
            server_path = CLASS_COMMON_UTIL.bytes_to_str(server_path)
            is_exists_server_path = os.path.exists(server_path)
            
            self.G_DEBUG_LOG.info('[exists]{}:{}'.format(server_path, is_exists_server_path))
            
            if is_exists_server_path:
                cmd_max = '{fastcopy_exe} /speed=full /force_close  /no_confirm_stop /force_start "{src}" /to="{dest}"'.format(
                    fastcopy_exe=r'c:\fcopy\FastCopy.exe',
                    src=server_path.replace('/', '\\'),
                    dest=target_path.replace('/', '\\'),
                )
                CLASS_COMMON_UTIL.cmd(cmd_max, my_log=self.G_DEBUG_LOG)
                
                local_file_path = os.path.join(target_path, os.path.basename(server_path))
                if os.path.isfile(local_file_path):
                    if local_file_path.endswith('.max.7z'):
                        self.MAX_ZIP_LIST.append(local_file_path)
                    elif local_file_path.endswith('.ifl'):
    ...
    def handle_ies(self):
        if self.G_ZONE == '2':  # foreign
            if self.IES_LIST:
                for ies_path in self.IES_LIST:
                    self.G_DEBUG_LOG.info(ies_path)
                    is_exists_server_path = os.path.exists(ies_path)
            
                    self.G_DEBUG_LOG.info('[exists]{}:{}'.format(ies_path, is_exists_server_path))
                    
                    if is_exists_server_path:
                        ies_change_flag = False
                        try:
                            with codecs.open(ies_path, 'r+', 'latin-1') as f:
                                f_string = f.read()
                                self.G_DEBUG_LOG.info('TEST1:%s' % f_string)
                                if f_string:
                                    ...
                                    self.G_DEBUG_LOG.info('TEST2:%s' % f_string)
                                    if ies_change_flag:
                                        iesPathBak = ies_path + '_bak'
                                        shutil.copy2(ies_path, iesPathBak)
                                        f.seek(0)
                                        f.write(f_string)
                        except Exception as e:
                            self.G_DEBUG_LOG.info('[warn]handle ies failed:%s' % ies_path)
    ...
    def get_max_file_client(self,source_max_file):
        ...
        if self.NET_RENDER:
            ...
            client_cg_file = self.G_TASK_JSON_DICT['task_info'].get('input_cg_file')
            cg_file = os.path.join(self.G_WORK_RENDER_TASK_MAX, client_cg_file.replace(':', ''))
        return cg_file
        ...
        if self.MAX_PREPROCESS:
            if not self.NOT_RUN_MAX:
                ...
                if self.G_CHANNEL == '2':
                    ...
                    max_file_node = os.path.normpath(self.get_max_file(cg_file))
                    max_file_node_new = os.path.normpath(os.path.join(os.path.dirname(max_file_node),(self.G_TASK_ID+'.max')))
                #rename max file
        ...
        if file_list:            
            for files in file_list:                
                ...
                if os.path.exists(files):
                    ...
                    local_file_path = os.path.join(target_path, os.path.basename(files))
                    if os.path.isfile(local_file_path):
                        if local_file_path.endswith('.max.7z'):
                            self.MAX_ZIP_LIST.append(local_file_path)
                        elif local_file_path.endswith('.ifl'):
    ...
    def copy_file_web_by_name(self):
        ...
        for files in asset_input_list:
            ...
            local_file_path = os.path.join(target_path, os.path.basename(files))
            if os.path.isfile(local_file_path):
                if local_file_path.endswith('.ifl'):
                    self.IFL_LIST.append(local_file_path)
                elif local_file_path.endswith('.ies'):
                    self.IES_LIST.append(local_file_path)
            ##try:
    ...
    def RB_HAN_RESULT(self):
        ...
        if not os.path.exists(self.G_TEMP_PATH):
            CLASS_COMMON_UTIL.make_dir(self.G_TEMP_PATH)
    ...
    def set_max_preprocess(self):
        ...
        is_exists_max_cmd_txt = os.path.exists(max_cmd_txt)
        self.G_DEBUG_LOG.info('[exists]{}:{}'.format(max_cmd_txt, is_exists_max_cmd_txt))
        
        if is_exists_max_cmd_txt:
        ...
        if 'miscellaneous' in self.G_TASK_JSON_DICT and self.G_TASK_JSON_DICT['miscellaneous'].get('render_type', None) == 'maxcmd':
            self.MAX_PREPROCESS=True
        if 'G_SCHEDULER_CLUSTER_NODES' in self.__dict__ and len(self.G_SCHEDULER_CLUSTER_NODES) > 0:
            self.MAX_PREPROCESS=True
        
        self.G_DEBUG_LOG.info('MAX_PREPROCESS={}'.format(self.MAX_PREPROCESS))
