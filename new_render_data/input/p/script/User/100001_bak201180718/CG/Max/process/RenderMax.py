...
class RenderMax(Max):
    ...
    def RB_PRE_RESET_NODE(self):
        """
        1.Delete grab(screenshot) for 3ds Max, Only windows.
        """
        self.G_DEBUG_LOG.info('[RenderMax.RB_PRE_RESET_NODE.start.....]')
        grab_server_path_big = os.path.join(self.G_GRAB_PATH, 'big', '{}_{}'.format(self.G_MUNU_ID, self.G_JOB_ID))
        grab_server_path_small = os.path.join(self.G_GRAB_PATH, 'small', '{}_{}'.format(self.G_MUNU_ID, self.G_JOB_ID))
        if os.path.exists(grab_server_path_big):
            self.G_DEBUG_LOG.info('Delete grab:%s' % grab_server_path_big)
            try:
                shutil.rmtree(grab_server_path_big)
            except Exception as e:
                self.G_DEBUG_LOG.info(e)
        else:
            self.G_DEBUG_LOG.info('The path is not exists:%s' % grab_server_path_big)

        if os.path.exists(grab_server_path_small):
            self.G_DEBUG_LOG.info('Delete grab:%s' % grab_server_path_small)
            try:
                shutil.rmtree(grab_server_path_small)
            except Exception as e:
                self.G_DEBUG_LOG.info(e)
        else:
            self.G_DEBUG_LOG.info('The path is not exists:%s' % grab_server_path_small)

        self.G_DEBUG_LOG.info('[RenderMax.RB_PRE_RESET_NODE.end.....]')
    ...
    def write_ms_file(self):
        ...
        ms_str = ms_str + self.G_ACTION_ID + '","' + self.G_CG_OPTION + '","' + self.G_ACTION + '","' + self.G_WORK_RENDER_TASK_OUTPUT_XXX.replace('\\', '/') + '/' + '","' + self.MAX_FILE.replace('\\', '/') + '", @"'
    ...
    def write_ms_file1(self):
        ...
        '''.format(
            ...
            task_output_xxx=self.G_WORK_RENDER_TASK_OUTPUT_XXX.replace('\\', '/'),
            max_file=self.MAX_FILE.replace('\\', '/'),
...
""" % (node_ip_num)
    ...
    def sort_pic_list(self, pic_list):
        ...
        pic_list_change = False
        for index, element in enumerate(pic_list):
            if output_file_basename in element:
                for single_frame in range(int(self.G_CG_START_FRAME), int(self.G_CG_END_FRAME) + 1, int(self.G_CG_BY_FRAME)):
                    frame_serial = self.generate_frame_serial(single_frame, count=4)
