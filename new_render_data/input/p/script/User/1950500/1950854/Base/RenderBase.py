...
class RenderBase(object):
    ...
    def __init__(self, **param_dict):
        ...
        if account_type == "2":
            # 父账号id
            parent_id = common["common"]
            # 是否共享父账号的资产存储  0，不共用，1，共用
            share_storage = common["share_storage"]
            if share_storage == "1":
                # 主子账号重新赋值
                self.G_INPUT_CG_FILE = self.G_INPUT_CG_FILE.replace(self.G_USER_ID, parent_id)
                self.G_INPUT_PROJECT_PATH = self.G_INPUT_PROJECT_PATH.replace(self.G_USER_ID, parent_id)
                self.G_INPUT_USER_PATH = self.G_INPUT_USER_PATH.replace(self.G_USER_ID, parent_id)
