...
class C4D(RenderBase):
    ...
    def setPluginByPyFile(self):
        self.G_PROCESS_LOG.info('[...Plugin is beginning...]')
        if os.path.exists(self.G_PLUGINS):
            ...
            if isinstance(configJson["plugins"], dict):
                ...
                for key in configJson["plugins"]:
                    ...
                    plugin.node = node
                    self.G_PROCESS_LOG.info('[...Plugin loaded, cmd= "B:\\plugins\\C4D\\script\\C4DPluginManager.py" "%s" "%s" "%s" "%s" ]' % (plugin.plugin_name, \
