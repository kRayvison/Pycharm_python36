...
class PreMax(Max):
    ...
    def RB_RENDER(self):#5
        ...
        if os.path.exists(net_render_txt):
            ...
            if self.G_CHANNEL == '1' or self.G_CHANNEL == '2':
                if self.G_ASSET_JSON_DICT.has_key('max'):                    
                    for max_file in self.G_ASSET_JSON_DICT['max']:
                        max_file = CLASS_COMMON_UTIL.str_to_unicode(max_file)
                        if os.path.exists(max_file):
                            target_path = self.G_WORK_RENDER_TASK_MAX
                            self.G_DEBUG_LOG.info('unicode')
                            cmd_max=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "'+max_file+'" /to="'+target_path+'"'
    ...
    def writeMsFile(self,msFile,sceneFile):
        ...
        clientArrayStr=clientArrayStr+self.G_ACTION_ID+'","'+sceneFile+'","'+self.G_PLATFORM+'","'+self.G_CG_OPTION+'","'+subFrom+'","'+self.G_TASK_JSON.replace('\\','/')+'","'+self.G_KG+'")\r\n'
    ...
    def copy_file_web_by_path(self):
        ...
        if self.G_ASSET_JSON_DICT.has_key('asset_input_server'):
            for asset in self.G_ASSET_JSON_DICT['asset_input_server']:
                asset = CLASS_COMMON_UTIL.str_to_unicode(asset)
        ...
        if self.G_ASSET_JSON_DICT.has_key('max'):
            ...
            for max_path in max_list:
                max_path = CLASS_COMMON_UTIL.str_to_unicode(max_path)
        ...
        if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['gi']=='1':
            if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('primary_gi_engine') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['primary_gi_engine']=='0':
                #--------------------------Irradiance map_____from file--------------------------
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('irradiance_map_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['irradiance_map_mode']=='2':
                ...
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('irradiance_map_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['irradiance_map_mode']=='7':
            ...
            elif self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('primary_gi_engine') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['primary_gi_engine']=='3':
            
                #--------------------------Light cache_____from file--------------------------
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('light_cache_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['light_cache_mode']=='2':
            ...
            #--------------------------Light cache_____from file--------------------------        
            if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('secondary_gi_engine') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['secondary_gi_engine']=='3':
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('light_cache_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['light_cache_mode']=='2':
                    print 'light cache from file'
                    light_cache_file = self.G_TASK_JSON_DICT['scene_info_render']['renderer']['light_cache_file']
                    # file_list.append(self.RENDERBUS_PATH.convert_to_renderbus_path(light_cache_file))
        ...
        if file_list:            
            for files in file_list:                
                ...
                if os.path.exists(files):
                    ...
                    self.G_DEBUG_LOG.info('unicode')
                    cmd_copy=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "'+files+'" /to="'+target_path+'"'
        ...
        if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['gi']=='1':
            if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('primary_gi_engine') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['primary_gi_engine']=='0':
                ...
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('irradiance_map_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['irradiance_map_mode']=='2':
                    ...
                    file_list.append(irrmapFile)
                #--------------------------Irradiance map_____Animation(rendering)--------------------------
                elif self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('irradiance_map_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['irradiance_map_mode']=='7':
                    print 'irrmapmode animation (rendering)'
                    animation_file = self.G_TASK_JSON_DICT['scene_info_render']['renderer']['irrmap_file']
                    # animation_file=self.RENDERBUS_PATH.convert_to_renderbus_path(animation_file)
                    self.getAnimationVrmap(file_list,animation_file)
    ...
    def copy_file2(self):
        ...
        if self.G_ASSET_JSON_DICT.has_key('max'):
            for asset_str in self.G_ASSET_JSON_DICT['max']:
                asset_str = CLASS_COMMON_UTIL.str_to_unicode(asset_str)
        ...
        if file_list:
            ...
            for files in file_list:
                ...
                if os.path.exists(file_path):
                    ...
                    self.G_DEBUG_LOG.info('unicode')
                    cmd_copy=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "'+file_path+'" /to="'+target_path+'"'
    ...
    def set_max_preprocess(self):
        ...
        if self.G_TASK_JSON_DICT['miscellaneous'].has_key('render_type') and self.G_TASK_JSON_DICT['miscellaneous']['render_type'] == 'maxcmd':
            self.MAX_PREPROCESS=True
        if self.__dict__.has_key('G_SCHEDULER_CLUSTER_NODES') and len(self.G_SCHEDULER_CLUSTER_NODES) > 0:
            self.MAX_PREPROCESS=True
    ...
    def set_vray0000(self):
        if self.__dict__.has_key('G_SCHEDULER_CLUSTER_NODES') and len(self.G_SCHEDULER_CLUSTER_NODES) > 0:
            if self.G_CG_CONFIG_DICT.has_key('plugins') and self.G_CG_CONFIG_DICT['plugins'].has_key('vray'):
                if self.G_CG_CONFIG_DICT['plugins']['vray'].startswith('1') or self.G_CG_CONFIG_DICT['plugins']['vray'].startswith('2'):
                    ...
                    vray_new = u'0000'
                    for k1,v1 in self.G_CG_CONFIG_DICT['plugins'].items():
                    ...
                    if not self.G_CG_CONFIG_DICT.has_key('old_plugins'):
