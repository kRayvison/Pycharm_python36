...
class RenderBase():
    ...
    def __init__(self,**param_dict):
        ...
        # self.G_SMALL_TASK_ID = '888'  #chen test
        # self.G_PHOTON_SMALL_TASK_ID = '999'  #chen test
        ...
        self.G_GRAB_PATH=os.path.normpath(self.G_TASK_JSON_DICT['system_info']['common']['grab_path'])
        
        # output_user_path
        self.G_OUTPUT_USER_PATH=os.path.normpath(self.G_TASK_JSON_DICT['system_info']['common']['output_user_path'])
        big_task_id_scene_name = self.G_OUTPUT_USER_PATH.split(os.sep)[-1]  #  31814_max2014_test_proxy
        small_task_id_scene_name = self.G_SMALL_TASK_ID + big_task_id_scene_name[len(self.G_TASK_ID):]  #  31815_max2014_test_proxy
        self.G_OUTPUT_USER_PATH = os.path.join(self.G_OUTPUT_USER_PATH, small_task_id_scene_name)
        self.G_BIG_SMALL = big_task_id_scene_name + os.sep + small_task_id_scene_name + os.sep  #  31814_max2014_test_proxy\31815_max2014_test_proxy\
        ##KAFKA
        ...
        if self.G_TASK_JSON_DICT['system_info']['common'].has_key('render_core_type'):
        ...
        if self.G_ACTION not in ['Analyze','Pre']:
            ...
            for root, dirs, files in list_dirs:
                for name in files:
                    ...
                    big_pic_list.append((self.G_BIG_SMALL + work_big_pic.replace(self.G_WORK_RENDER_TASK_OUTPUT + os.sep,'')).replace('\\','/'))  #kafka message must use '/' , No '\'
    ...
    def RB_FEE(self):
        if self.G_ACTION not in ['Analyze','Pre']:
            ...
            try:
                self.G_FEE_PARSER.write(codecs.open(self.G_FEE_FILE, 'w', 'utf-8'))
        ...
        if os.path.exists(dir):
            ...
            for root, dirs, files in list_dirs: 
                for name in files:
                    ...
                    small_pic_list.append((self.G_BIG_SMALL + small_pic).replace('\\','/'))
