...
class RenderMaya(Maya):
        ...
        if int(self.G_CG_TILE_COUNT) != int(self.G_CG_TILE):
            ...
            if self.g_one_machine_multiframe is True:
                if self.G_RENDER_CONTINUE == "true":
                    ...
                    # 重新渲染
                    print("self.G_RECOMMIT_FLAG == {} and self.G_RENDER_CONTINUE == {}".format(self.G_RECOMMIT_FLAG,
                                                                                               self.G_RENDER_CONTINUE))
    ...
    def handle_file_bak(self, frame):
        ...
        self.G_DEBUG_LOG.info('[Maya.result_action.end.....]')
    ...
    def get_render_cmd(self):
        ...
        if self.renderSettings["tile_region"]:
            ...
            if self.renderer in ["mentalRay", "arnold", "vray"]:
                ...
                elif self.renderer == "mentalRay" and float(self.CG_VERSION) > 2016.5 and "mentalray" in self.CG_PLUGINS_DICT:
                    cmd += " -r mr -reg %(tile_region)s" % self.renderSettings
