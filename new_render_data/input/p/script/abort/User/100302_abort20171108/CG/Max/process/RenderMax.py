...
class RenderMax(Max):
    ...
    def RB_CONFIG(self):
        ...
        if self.G_TASK_JSON_DICT['scene_info_render']['common'].has_key('gamma'):
        ...
        try:
            ...
            with open(license_env_path,'r') as pl:
                ...
                if self.G_CG_CONFIG_DICT.has_key('plugins'):
                    for plugins_key in self.G_CG_CONFIG_DICT['plugins'].keys():
                        plugin_str = plugins_key + self.G_CG_CONFIG_DICT['plugins'][plugins_key]
                        if pl_dict.has_key(plugin_str):
    ...
    def write_ms_file(self):
        ...
        ms_str=ms_str+self.G_ACTION_ID+'","'+self.G_CG_OPTION+'","'+self.G_ACTION+'","'+self.G_WORK_RENDER_TASK_OUTPUT.replace('\\','/')+'","'+self.MAX_FILE+'", @"'
        ...
        self.G_DEBUG_LOG.info('[RenderMax.writeMsFile.end.....]')
        return task_ms_file   
    ...
    def result_action_photon(self):
        if self.G_ONLY_PHOTON == '1':
            ...
            upload_path = os.path.join(self.G_INPUT_PROJECT_PATH,'photon',self.G_TASK_ID)     
        ...
        try:
            ...
            print e
        # CLASS_FRAME_CHECKER.main(self.G_WORK_RENDER_TASK_OUTPUT,upload_path,my_log=self.G_DEBUG_LOG)
    ...
    def get_light_cache_frame(self):
        ...
        if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['gi']=='1':
            ...
            if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('light_cache_mode') and self.G_TASK_JSON_DICT['scene_info_render']['renderer']['light_cache_mode']=='1':
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi_frames'):
    ...
    def get_render_cmd(self,task_ms_file):
        ...
        if self.MAX_CMD_RENDER==True:
            if self.G_CHANNEL == '1' or self.G_CHANNEL == '2':  #web
                ...
                if self.G_TASK_JSON_DICT['scene_info']['common'].has_key('width'):
                    cmd_render_width = self.G_TASK_JSON_DICT['scene_info']['common']['width']
                if self.G_TASK_JSON_DICT['scene_info']['common'].has_key('height'):
            ...
            if self.G_ACTION == 'RenderPhoton':
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi_width'):
                    cmd_render_height = self.G_TASK_JSON_DICT['scene_info_render']['renderer']['gi_width']
                if self.G_TASK_JSON_DICT['scene_info_render']['renderer'].has_key('gi_height'):
            ...
            render_cmd=max_exe+' -start:'+self.G_CG_START_FRAME+' -end:'+self.G_CG_END_FRAME+' -nthFrame:'+self.G_CG_BY_FRAME+' -o:"'+self.G_WORK_RENDER_TASK_OUTPUT.replace('\\','/')+'/'+cmd_render_output+'" -camera:"'+self.G_CG_OPTION+'" -w:'+cmd_render_width+' -h:'+cmd_render_height+' -continueOnError '
