...
class RenderBase():
            ...
            if not os.path.exists(resource_monitor):
                ...
                if self.G_ACTION not in ['Analyze','Pre'] and self.G_CG_NAME in cg_process_dict:
                    ...
                    if len(process_name_list) <= 0:
                        ...
                        command = r'%s "%s" "%s|%s|%s|%s|%s"' % (resource_monitor, process_name, render_frame, self.G_TASK_ID, self.G_USER_ID, cg_flag, plugin_string)
    ...
    def rendering_copy_notify(self, start_or_end='0'):
        ...
        request = urllib2.Request(url=rendering_copy_interface, headers=headers, data=json.dumps(data))
        try:
