...
class RenderMax(Max):
            ...
            for root, dirs , files in os.walk(photon_work_path):
                for name in files:
                    if name.endswith('vrmap') or name.endswith('vrlmap'):
                        ...
                        photon_file_new = os.path.join(root,name.replace(id_from, id_to))
        ...
        if  self.G_KG=='100' or self.G_KG=='101' or self.G_KG=='102':#inc
            if self.G_ACTION == 'Render':
                ...
                if self.G_MULTI_CAMERA:
                    photon_project_path=os.path.join(self.G_INPUT_PROJECT_PATH,'photon',self.G_PHOTON_SMALL_TASK_ID,self.G_CG_OPTION)
                    photon_work_path=os.path.join(self.G_WORK_RENDER_TASK_MAX,'photon',self.G_CG_OPTION)
    ...
    def socket_send(self,final_data):
        try:
            ...
            if recv_data!=None and recv_data.endswith('success'):
                return True
            else:
                return False
        except Exception,e:
