...
class RenderBase():
    ...
    def RB_RESOURCE_MONITOR(self):
        ...
        if not os.path.exists(cg_process_path):
            ...
            cg_process_dict = eval(open(cg_process_path, 'r').read())
    
            #xxx.exe "3dsmax.exe" "1-5	19126418	1868556	3dsmax	vray,multiscatter"
            if self.G_RENDER_OS != '0':
                resource_monitor = ur'C:\work\munu_client\resource_monitor\resource_monitor.exe'  #1
                if not os.path.exists(resource_monitor):
                    ...
                    if self.G_ACTION not in ['Analyze','Pre'] and self.G_CG_NAME in cg_process_dict:
                        ...
                        if len(process_name_list) <= self.G_CG_PROCESS_FLAG:
                            self.G_DEBUG_LOG.info('[warn]The list of key %s in the %s index out of range!!!' % (self.G_CG_NAME,cg_process_path))
                        else:
                            ...
                            t = threading.Thread(target=self.monitor_cmd,args=(command,))
                            t.daemon = True
                            t.start()
    ...
    def monitor_cmd(self,command):
        ...
        register(self.monitor_exit)
        os.system(command)
    
    def monitor_exit(self):
        self.G_DEBUG_LOG.info('[monitor_exit]start...')
        os.system('taskkill /F /IM resource_monitor.exe /T')
