...
class AnalyzeMax(Max):
    ...
    def han_task_json(self,node_task_json):
        ...
        if  self.ANALYZE_TXT_PARSER.has_section(common_section):
            for option in self.ANALYZE_TXT_PARSER.options(common_section):  
                print " ",option,"=",self.ANALYZE_TXT_PARSER.get(common_section,option)
                if option=='all_camera':
                    all_camera_str=self.ANALYZE_TXT_PARSER.get('common','all_camera')
                    scene_info_common_dict['all_camera']=self.str_to_list(all_camera_str,'[,]')
                elif option=='renderable_camera':
                    renderable_camera_list=[]
                    renderable_camera_str=self.ANALYZE_TXT_PARSER.get('common','renderable_camera')
                    renderable_camera_list.append(renderable_camera_str)
                    scene_info_common_dict['renderable_camera']=renderable_camera_list
                elif option=='all_element':
                    all_element_str=self.ANALYZE_TXT_PARSER.get('common','all_element')
                    scene_info_common_dict['all_element']=self.str_to_list(all_element_str,'[,]')
                else:
                    if self.ANALYZE_TXT_PARSER.get(common_section,option)=='true':
                        scene_info_common_dict[option]='1'
                    elif self.ANALYZE_TXT_PARSER.get(common_section,option)=='false':
                        scene_info_common_dict[option]='0'
                    else:
                        scene_info_common_dict[option]=self.ANALYZE_TXT_PARSER.get(common_section,option)
        ...
        if  self.ANALYZE_TXT_PARSER.has_section(renderer_section):
            for option in self.ANALYZE_TXT_PARSER.options(renderer_section):  
                print " ",option,"=",self.ANALYZE_TXT_PARSER.get(renderer_section,option) 
                scene_info_renderer_dict[option]=self.ANALYZE_TXT_PARSER.get(renderer_section,option)  
    ...
    def han_tips_json(self,node_tips_json,scene_info_dict,asset_json_dict):
        ...
        if self.ANALYZE_TXT_PARSER.has_section('tips'):
            ...
            #for item_key in item_key_list:
                item_val = self.ANALYZE_TXT_PARSER.get('tips', item_key)
        ...
        if self.ANALYZE_TXT_PARSER.has_option('common', 'all_camera'):        
            camera_list = self.str_to_list(self.ANALYZE_TXT_PARSER.get('common', 'all_camera'),'[,]')
            if self.isDuplicateList(camera_list):
                tips_json_dict[self.TIPS_CODE_DICT['camera_duplicat']]=camera_list
            
        #------------check duplicate elements-----------   
        if self.ANALYZE_TXT_PARSER.has_option('common', 'all_element'):
            elem_list = self.str_to_list(self.ANALYZE_TXT_PARSER.get('common', 'all_element'),'[,]') 
        ...
        if self.ANALYZE_TXT_PARSER.has_section('vrmesh'):
            ...
            for index,item_key in enumerate(item_key_list):
                item_val = self.ANALYZE_TXT_PARSER.get('vrmesh', item_key)
                if item_val==None or item_val=='':
                    continue
                item_val = item_val.strip()
        ...
        if self.ANALYZE_TXT_PARSER.has_option('vray', 'vfb') and self.ANALYZE_TXT_PARSER.get('vray', 'vfb') =='true':
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'rendrawimgname') and self.ANALYZE_TXT_PARSER.get('vray', 'rendrawimgname') =='true':
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'rawimgname') and self.ANALYZE_TXT_PARSER.get('vray', 'rawimgname') =='':
                    tips_json_dict[self.TIPS_CODE_DICT['vrimg_undefined']]=[]
        
        #-----------\"Save separate render channels Checked but channels file is error"------------ 
        if self.ANALYZE_TXT_PARSER.has_option('vray', 'savesepchannel') and self.ANALYZE_TXT_PARSER.get('vray', 'savesepchannel') =='true':
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'channelfile') and self.ANALYZE_TXT_PARSER.get('vray', 'channelfile') =='':
                tips_json_dict[self.TIPS_CODE_DICT['channel_file_undefined']]=[]
                
        self.G_DEBUG_LOG.info('------------------------------ han tips gi---------------------------------') 
        #--------------------check gi------------------- 
        if self.ANALYZE_TXT_PARSER.has_option('vray', 'gi') and self.ANALYZE_TXT_PARSER.get('vray', 'gi')=='true' :
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'primarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'primarygiengine') =='0':
                #-----------Irradiance map mode :  \"Multiframe incremental\" not supported------------
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'irradiancemapmode') and self.ANALYZE_TXT_PARSER.get('vray', 'irradiancemapmode') =='1':
                    tips_json_dict[self.TIPS_CODE_DICT['multiframe_notsupport']]=[]
                    
                #-----------Irradiance map mode : Add to current map not supported------------
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'irradiancemapmode') and self.ANALYZE_TXT_PARSER.get('vray', 'irradiancemapmode') =='3':
                    tips_json_dict[self.TIPS_CODE_DICT['addtocmap_notsupport']]=[]
            
            #-----------Light cache mode : \"Progressive path tracing\" not supported------------        
            elif self.ANALYZE_TXT_PARSER.has_option('vray', 'primarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'primarygiengine') =='3':
                 if self.ANALYZE_TXT_PARSER.has_option('vray', 'lightcachemode') and self.ANALYZE_TXT_PARSER.get('vray', 'lightcachemode') =='3':
                    tips_json_dict[self.TIPS_CODE_DICT['ppt_notsupport']]=[]
                    
            #-----------Light cache mode : \"Progressive path tracing\" not supported------------ 
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'secondarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'secondarygiengine') =='3':
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'lightcachemode') and self.ANALYZE_TXT_PARSER.get('vray', 'lightcachemode') =='3':
                    tips_json_dict[self.TIPS_CODE_DICT['ppt_notsupport']]=[]
                    
            #-----------Irradiance map mode : \"From file\" VRMAP_MISSING------------ 
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'primarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'primarygiengine') =='0':
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'irradiancemapmode') and self.ANALYZE_TXT_PARSER.get('vray', 'irradiancemapmode') =='2':
                    if self.ANALYZE_TXT_PARSER.has_option('vray', 'irrmapfile') and self.ANALYZE_TXT_PARSER.get('vray', 'irrmapfile') =='':
                        tips_json_dict[self.TIPS_CODE_DICT['vrmap_missing']]=[]
            
            #-----------Irradiance map mode : \"Animation rendering\" ANIMATION_MAP_MISSING------------ 
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'primarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'primarygiengine') =='0':
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'irradiancemapmode') and self.ANALYZE_TXT_PARSER.get('vray', 'irradiancemapmode') =='7':
                    if self.ANALYZE_TXT_PARSER.has_option('vray', 'irrmapfile') and self.ANALYZE_TXT_PARSER.get('vray', 'irrmapfile') =='':
                        if self.ANALYZE_TXT_PARSER.has_option('vray', 'irrmapfile') and self.ANALYZE_TXT_PARSER.get('vray', 'irrmapfile') =='':
            ...
            if self.ANALYZE_TXT_PARSER.has_option('vray', 'secondarygiengine') and self.ANALYZE_TXT_PARSER.get('vray', 'secondarygiengine') =='3':
                if self.ANALYZE_TXT_PARSER.has_option('vray', 'lightcachemode') and self.ANALYZE_TXT_PARSER.get('vray', 'lightcachemode') =='2':
                    if self.ANALYZE_TXT_PARSER.has_option('vray', 'lightcachefile') and self.ANALYZE_TXT_PARSER.get('vray', 'lightcachefile') =='':
        ...
        if len(dup_texture_dict)>0 :
            dup_texture_list=dup_texture_dict.values()
            dup_str=''
            total_dup_list=[]
        ...
        for key,value in self.TIPS_CODE_DICT.items():
    ...
    def maxfind(self):
        ...
        if self.G_MAXFIND_RENDERER==None or self.G_MAXFIND_MAX_VERSION_STR==None:
            tips_str='{'+str(self.TIPS_CODE_DICT['maxinfo_failed'])+':[]}'
    ...
    def multiscatterAndVray(self):
        ...
        if self.G_CG_CONFIG_DICT.has_key('plugins'):
            plugin_dict=self.G_CG_CONFIG_DICT['plugins']
            if plugin_dict.has_key('multiscatter'):
    ...
    def config_max_plugin(self):#config 3ds Max ,vray,plugin...and so on
        ...
        if not self.G_CG_CONFIG_DICT.has_key('cg_name'):
            self.G_CG_CONFIG_DICT['cg_name']='3ds Max'
            
        if not self.G_CG_CONFIG_DICT.has_key('cg_version'):
    ...
    def get_missing_code_info(self,info_code,info_type,asset_json_dict,tips_json_dict):
        ...
        tip_info_list=[]
        #if self.G_TASK_JSON_DICT.has_key('software_config'):
        if asset_json_dict.has_key(info_type):
    ...
    def getItemFromSection(self, section):
        ...
        if self.ANALYZE_TXT_PARSER.has_section(section):
            ...
            for item_key in item_key_list:
                # print item_key
                item_path = self.ANALYZE_TXT_PARSER.get(section, item_key).strip()
                # print item_path
                #if ignore:
                #    rbFile = item_path 
                #else:
    ...
    def get_file_from_project_by_name(self,file):
        ...
        if self.G_INPUT_PROJECT_ASSET_DICT.has_key(file_base_name):
    ...
    def get_point_cache_list(self,asset_file,result_list):
        ...
        if wild_path.endswith('.xml') and wild_path2!=None and os.path.exists(wild_path2):
            ...
            if m != None:
                cache_start = int(m.group(1)) / int(cacheTimePerFrame)
                cache_end  = int(m.group(2)) / int(cacheTimePerFrame)
            else:
    ...
    def get_all_asset(self):
        ...
        # all assets
        dict_list = map(self.handleAsset(asset_dict), asset_dict)
        for dict in dict_list:
            self.G_DEBUG_LOG.info('------------------------------ start---------------------------------') 
            for asset_type, asset_list in dict.items():
