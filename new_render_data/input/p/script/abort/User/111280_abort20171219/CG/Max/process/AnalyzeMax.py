...
class AnalyzeMax(Max):
    ...
    def get_file_path(self,file):
        if self.ASSET_WEB_COOLECT_BY_PATH:
            return self.renderbus_path_obj.convert_path(file)
        else:
            return self.get_file_from_project_by_name(file)
            ...
            if key == 'fumefx':
                for asset in asset_list:
                    ...
                    for i in range(start , end+1, -1 if start > end else 1):
                        ...
                        result_list.append(self.get_file_path(file)) 
            elif key == 'xmesh':
                print '-----------------------xmesh--------------------------'
                for asset in asset_list:
                    print asset
                    ...
                    if render_sequence!=None and render_sequence!='':
                        ...
                        renderSequenceFolder=os.path.dirname(self.get_file_path(render_sequence))
                    ...
                    if proxy_sequence!=None and proxy_sequence!='':
                        ...
                        proxySequenceFolder=os.path.dirname(self.get_file_path(proxy_sequence))
               ...
               for asset in asset_list:
                   ...
                   for i in range(start , end+1, -1 if start > end else 1):
                        if i >= 0:
                            ...
                            serial = str((sys.maxint+1)*2+i)
                        # result_list.append(self.renderbus_path_obj.convert_path(file.replace('*',serial))) 
                        result_list.append(self.get_file_path(file.replace('*',serial))) 
                 ...
                 for asset in asset_list:
                    ...
                    if wild_path.endswith('.vrmesh'):
                        ...
                        file_path = self.get_file_path(wild_path)
                ...
                for asset in asset_list:
                    if asset.endswith('.ifl'):
                        ...
                        result_list.append(self.get_file_path(asset))
