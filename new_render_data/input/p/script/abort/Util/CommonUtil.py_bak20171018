...
class RBCommon(object):
    ...
    def cmd(self,cmd_str,my_log=None,try_count=1,continue_on_error=False,my_shell=False,callback_func=None):#continue_on_error=true-->not exit ; continue_on_error=false--> exit 
        ...
        while l < try_count:
            ...
            my_popen.stdin.write('3/n')
            my_popen.stdin.write('4/n')
    ...
    def python_move(self, move_source, move_target, continue_on_error=False, my_shell=False):
        ...
        move_source = self.str_to_unicode(move_source)
        move_target = self.str_to_unicode(move_target)
        try:
    ...
    def mount_path(self,dict={}):
        ...
        for key in dict.keys():
            ...
            if not os.path.exists(key):
                os.makedirs(key)
            path = dict[key]['path']
            if dict[key].has_key('username') and dict[key].has_key('password'):
    ...
    def write_hosts(self,server_dict):
        ...
        ip_list=[]
        server_list=[]
        for key_ip,value_server in server_dict.items():
            print key_ip
...
class RBKafka():
    ...
    @classmethod        
    def produce(self,json_data, my_kafka_host, my_kafka_port, my_kafka_topic):
        flag = False
        j = 0
        self.producer = KafkaProducer(bootstrap_servers = '{kafka_host}:{kafka_port}'.format(kafka_host=my_kafka_host,kafka_port=my_kafka_port))
