...
maxon_data_path = r'C:\\users\\enfuzion\\AppData\\Roaming\\MAXON\\CINEMA 4D %s\\plugins\\'
...
class C4dLoader(object):
    ...
    def execute(self):
        ...
        #03.run c4d
        c4d_cmdline = '"C:/Program Files/MAXON/%s/CINEMA 4D 64 Bit.exe" -task_id=%s -cg_file="%s" -task_json="%s" -asset_json="%s" -tips_json="%s"' \
    ...
    def __copy_file(self, src, dst):
        ...
        print (r'C:/fcopy/FastCopy.exe /force_close "' + src + '" /to="' + dst + '"')
        os.system(r'C:/fcopy/FastCopy.exe /force_close "' + src + '" /to="' + dst + '"')
