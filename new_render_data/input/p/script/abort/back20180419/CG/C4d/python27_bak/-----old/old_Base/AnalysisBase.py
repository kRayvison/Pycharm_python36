...
class AnalysisBase():
    ...
    def __init__(self,**paramDict):
        ...
        if 'G_RENDEROS' in paramDict:
            ...
            if self.G_RENDEROS=='Linux':
                self.G_LOG_WORK='/tmp/nzs-data/log/helper'
    ...
    def RBinitLog(self):#2
        ...
        newdebugLog=os.path.join(analyseLogPath,(self.G_JOB_NAME+'_debug.log'))
        ...
        newdebugLogHandler=logging.FileHandler(newdebugLog)
        newdebugLogHandler.setFormatter(fm)
        self.G_ANALYSE_LOG.addHandler(newdebugLogHandler)
    ...
    def RBhanResult(self):#8
        ...
        tipsJson=os.path.join(self.G_WORK_TASK,'tips.json')
        self.G_ANALYSE_LOG.info('tipsJson...'+tipsJson)
        self.G_ANALYSE_LOG.info('G_PATH_INPUTTXT...'+self.G_PATH_INPUTTXT)
        if  os.path.exists(tipsJson):
            self.pythonCopy(tipsJson,self.G_PATH_INPUTTXT)
    ...
    def RBcmd(self,cmdStr,continueOnErr=False,myShell=1,myLog=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
        if myLog:
