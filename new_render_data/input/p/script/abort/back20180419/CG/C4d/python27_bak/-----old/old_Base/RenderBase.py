    ...
    def __init__(self,**paramDict):
        ...
        if 'G_RENDEROS' in paramDict:
            ...
            if self.G_RENDEROS=='Linux':
                ...
                self.G_RENDER_WORK='/tmp/nzs-data/work/render'
        ...
        self.G_RAYVISION_PLUGINSCFG=os.path.join(self.G_RENDER_WORK_TASK_CFG,'plugins.cfg')
...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
        ...
        '''
        os.environ["G_JOB_NAME"] = self.G_JOB_NAME
    ...
    def RBinitLog(self):#2
        ...
        processLogPath=os.path.join(processLogDir,(self.G_JOB_NAME+'_process2.txt'))
        ...
        simpleLogPath=os.path.join(processLogDir,(self.G_JOB_NAME+'_render2.txt'))
        self.G_SIMPLE_LOG.setLevel(logging.DEBUG)
        self.simpleLogHandler=logging.FileHandler(simpleLogPath)
        ...
        renderLogPath=os.path.join(renderLogDir,(self.G_JOB_NAME+'_max2.txt'))
        ...
        newdebugLogPath=os.path.join(processLogDir,(self.G_JOB_NAME+'_debug.log'))
        newdebugLogHandler=logging.FileHandler(newdebugLogPath)
        newdebugLogHandler.setFormatter(fm)
        self.G_PROCESS_LOG.addHandler(newdebugLogHandler)
        self.G_SIMPLE_LOG.addHandler(newdebugLogHandler)
    ...
    def RBlog(self,logStr,logType='common'):
        ...
        if logType!='system':
            logStr=self.convertUnicode2Str(logStr)
    ...
    def RBhanFile(self):#3 copy script,copy from pool,#unpack
        ...
        self.G_PROCESS_LOG.info('[BASE.RBhanFile.start.....]'+self.G_RENDERTYPE)
        ...
        if  os.path.exists(dir):
            ...
            for root, dirs, files in list_dirs: 
                for name in files:
                    ...
                    smallTmpName = self.G_JOB_NAME+'_tmp.jpg'
                    workSmallPic=os.path.join(workSmallPath,smallName)
                    workSmallTmpPic=os.path.join(workSmallPath,smallTmpName)


                    workBigPicTmp=os.path.join(root, 'tmp'+os.path.splitext(workBigPic)[1] )
                    try:
                        os.rename(workBigPic,workBigPicTmp);
                    except Exception, e:
                        self.G_PROCESS_LOG.info('rename failed-----')
                        pass
                    if not os.path.exists(workBigPicTmp):
                        workBigPicTmp = workBigPic
                        workSmallTmpPic = workSmallPic

                    oiioPath='c:\\oiio\\OpenImageIO-1.5.18-bin-vc9-x64\\oiiotool.exe'
                    convertCmd='c:/ImageMagick/nconvert.exe  -out jpeg -ratio -resize '+smallSize+' 0 -overwrite -o "'+workSmallTmpPic +'" "'+workBigPicTmp+'"'
                    if ext=='.exr' and os.path.exists(oiioPath):
                        self.G_PROCESS_LOG.info('exr parse----'+name)
                        convertCmd=oiioPath+' "'+workBigPicTmp+'" -resize '+self.getConvertR(workBigPicTmp,oiioPath)+' -o "'+workSmallTmpPic+'"'
                    #print workBigPic
                    try:
                        self.RBcmd(convertCmd,True,False)
                    except Exception, e:
                        self.G_PROCESS_LOG.info('parse smallPic failed-----')
                        pass
                    if not workBigPicTmp==workBigPic:
                        os.rename(workBigPicTmp,workBigPic)
                        if os.path.exists(workSmallTmpPic):
                            os.rename(workSmallTmpPic,workSmallPic)
                    
                    self.G_PROCESS_LOG.info('workSmallTmpPic---'+workSmallTmpPic+'---workSmallPic---'+workSmallPic+'---workBigPicTmp---'+workBigPicTmp+'---workBigPic---'+workBigPic)
                    self.G_FEE_LOG.info('smallPic='+smallName)

    def getConvertR(self,path,oiiopath):
        try:
            cmd=oiiopath+' -info "'+path+'"'
            smallSize = '200x0'
            cmdp = subprocess.Popen(cmd,shell = True,stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
            while cmdp.poll()==None:
                resultLine = cmdp.stdout.readline().strip()
                if resultLine!='':
                    print "-"+resultLine
                    infoArray = resultLine[resultLine.rfind(':')+1:resultLine.find(',')].split("x")
                    print infoArray
                    if int(infoArray[0])<int(infoArray[0]):
                        smallSize = '0x200'
        except Exception, e:
    ...
    def pluginServer(self):
        self.RBcmd('net use "B:" "\\\\10.60.100.151\\td"')
