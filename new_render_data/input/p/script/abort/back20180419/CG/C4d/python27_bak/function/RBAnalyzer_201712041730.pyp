...
class C4dParser():
    ...
    def init(self):
        ...
        with open(self.task_json, 'r') as json_file:
            ...
            if not self.task_json_dict['scene_info'].has_key('common'):
    ...
    def analyze(self, *args):
        ...
        """
        # _Phy_S = self.get_Render_Settings_Physical_Sampler(doc)
        # _Phy_S_Pro_Mode = self.get_Render_Settings_Physical_Sampler(doc)
        # self.scene_common_dict['Render_Settings_Physical_Sampler'] = _Phy_S
        # self.scene_common_dict['Render_Settings_Physical_Sampler'] = _Phy_S_Pro_Mode
    ...
    # def get_Render_Settings_Physical_Sampler(self, doc):
    ...
    #     print Phy_S[int(vp[c4d.VP_XMB_RAYTRACING_SAMPLER])]
    #     #.write("Phy_S:"+Phy_S[int(vp[c4d.VP_XMB_RAYTRACING_SAMPLER])]+"\n\n")
    #     Phy_S_Pro_Mode = {0:'Infinite',1:'Pass Count',2:'Time Limit'}
    #     print Phy_S_Pro_Mode[int(vp[c4d.VP_XMB_RAYTRACING_INTERACTIVE_MODE])]
    #     #f.write("Phy_S_Pro_Mode:"+Phy_S_Pro_Mode[int(vp[c4d.VP_XMB_RAYTRACING_INTERACTIVE_MODE])]+"\n\n")
    #     return _Phy_S
    #     return _Phy_S_Pro_Mode
    ...
    def parse_realflow(self):
        print '[.HL_parse_realflow]'
        #1.场景内的缓存路径
        if os.path.exists(self.asset_json_dict(_realflow_mesh)):
            print 'rrrrrrrrrrrr' + _realflow_mesh
        #2.工程目录的路径
    ...
    def parse_analyze_result(self):
        self.parse_error(self.asset_json_dict['texture'])
        # parse_mesh
        # ...
    ...
    def parse_error(self, textures):
        ...
        for texture in textures:
            ...
            if str(fps) != str(Render_Settings_fps):
                self.FPS_check.append(fps)
                self.FPS_check.append(Render_Settings_fps)
            else:
                pass
