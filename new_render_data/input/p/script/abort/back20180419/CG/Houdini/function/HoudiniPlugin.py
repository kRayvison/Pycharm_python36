...
class HoudiniPlugin(object):
    ...
    def BaseDataSetup(cls,cls_in=''):
        ...
        cls_in.user_id = cls_in.G_TASK_JSON_DICT['system_info']['common']['user_id']
    ...
    def TipsInfo(cls,cls_in=''):
        ...
        if "Miss" in cls_in.asset_adict:
            keyword = error_key_adict["filemiss"] if "filemiss" in error_key_adict else '50001'   ### just for warnning
            infos_list = []
            ...
            cls_in.tips_adict[keyword] = infos_list

        if "Cam" in cls_in.asset_adict:
            HoudiniUtil.print_times("-"*23 + " Cameras Info " + "-"*23)
            keyword = error_key_adict["cmamiss"] if "cmamiss " in error_key_adict else '50002'   ### just for error
            infos_list = []
            ...
            if "miss" in cls_in.asset_adict["Cam"]:
                ...
                for keys in cls_in.asset_adict["Cam"]["miss"]:
                    asset_cam_adict = cls_in.asset_adict["Cam"]["miss"][keys]
                    if len(asset_cam_adict):
                        node_cunt = 1
                        for nodes in asset_cam_adict:
                            node_temp = asset_cam_adict[nodes][0]
                            if len(node_temp)>1:
                                if node_cunt==1:
            ...
            for elm in AllNode:
                if elm.type().name() in cls_in.render_node_type:
                    ...
                    if hou.node(cam_node):
                        ...
                        ''' miss '''
                        HoudiniUtil.asset_adict_edit(cls_in.asset_adict,"Cam","miss",cam_node,{cam_node:[['None'],rop_path]})
