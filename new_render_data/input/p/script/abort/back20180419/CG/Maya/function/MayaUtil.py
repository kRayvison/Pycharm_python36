...
class NukeMerge(dict):
    ...
    def get_input_output(self):
        ...
        print (all_files)
        print("2222222222")
        self.tile_files = [[re.sub(r'(.+/temp_tile/\d+/\d+)(.+)', r"\1/%s/\3" % (index), i.replace("\\", "/"))
    ...
    def render(self):
        for i in self.tile_files:
            ...
            # print (i[0])
            # print("2222222222")
            tile_output = self.renderSettings["output"] + re.sub(r'(.+/temp_tile/\d+/\d+)(.+)', r"\3", i[0])
            print(tile_output)
    ...
    def merge_files(self, tiles, input_files, tile_output):
        ...
        merge_images = os.path.join(self["func_path"], "merge_images_new.py")
        ...
        print (cmd)
        CLASS_COMMON_UTIL.cmd(cmd, my_log=None, continue_on_error=True, my_shell=True)
