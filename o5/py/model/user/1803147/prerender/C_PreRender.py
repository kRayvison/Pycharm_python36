...
def main(*args):
    ...
    for i in pm.ls(type="aiOptions"):
        if i.hasAttr("log_verbosity"):
            ...
            print "Your scens originalvray log_verbosity is %s " % (log_verbosity)
            if "mtoa" in info_dict["plugins"]:
                if info_dict["plugins"]["mtoa"] >= "3.0":
                    i.log_verbosity.set(2)
                    print "You scene mtoa version highter than 3.0,set log_verbosity to 2"
                else:
                    i.log_verbosity.set(1)
                    print "You scene mtoa version lower than 3.0,set log_verbosity to 1"
        if i.hasAttr("autotx"):
            i.autotx.set(False)
        if i.hasAttr("textureMaxMemoryMB"):
            i.textureMaxMemoryMB.set(20480)
        if info_dict["task_id"] in [10391272]:
