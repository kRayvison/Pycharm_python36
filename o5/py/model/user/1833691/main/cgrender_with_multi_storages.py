...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        # if self["common"]["userId"] not in [1911075,1909920,1818014, 1844854, 1875231, 1846887, 1854355, 1814593, 963931, 1873017,
                                            # 1812148, 1812148, 1830380, 1871175, 1899488, 1820991]:
            # cmd += " -preRender \"python \\\"pre_render_dict=%s;execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (pre_render_dict,PreRender)

        if self["common"]["userId"] in [1833691]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;start=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/user/1833691/prerender_test/C_PreRender.py\\\\\\\")\\\"\"" % (
