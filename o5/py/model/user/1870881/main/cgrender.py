...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] not in [1870881]:
        ...
        if self["common"]["userId"] in [1843630]:
            ...
            cmd = "\"%(bat)s\" %(user_id)s %(task_id)s \"%(render.exe)s\" \"%(renderableCamera)s\" " \
                ...
                % self["renderSettings"]
        print  self["renderSettings"]["width"]
        print  self["renderSettings"]["height"]
        
        if self["common"]["userId"] in [1923686]:
            cmd = "\"C:\Program Files\Autodesk\Maya2017\\bin\maya.exe\" -prompt -command \"workspace -fr \\\"images\\\" \\\"%s\\\";setAttr \\\"vraySettings.animBatchOnly\\\" 0; vrayShowDefaultRenderView; int $i; for($i=%d;$i<%d;$i++ ){setAttr \\\"defaultRenderGlobals.startFrame\\\" $i;setAttr \\\"defaultRenderGlobals.endFrame\\\" $i; currentTime $i; setAttr \\\"vraySettings.width\\\" %s; setAttr \\\"vraySettings.height\\\" %s;vrend -camera camera_renderShape -layer color;};quit -force;  \" -file %s" %(self["renderSettings"]["output"],self["renderSettings"]["start"],self["renderSettings"]["end"] + 1,self["renderSettings"]["width"],self["renderSettings"]["height"],self["renderSettings"]["maya_file"]) 
            ...
            if isinstance(cmd, list):
                ...
                for line in RvOs.run_command(cmd):
                    ...
                    if line_str:
                        ...
                        if "ASSERT FAILED" in line_str or "failed to render" in line_str or "DL inf loop detected" in line_str or "Render interrupted" in line_str \
                            or  "There was a fatal error building the scene for V-Ray" in line_str or  "Stack trace:" in line_str  or "Could not obtain a license" in line_str:
                            time.sleep(10)
                            RvOs.kill_children()
                            is_complete = 0
                            break
