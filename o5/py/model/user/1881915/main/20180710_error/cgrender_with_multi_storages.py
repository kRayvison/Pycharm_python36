...
class Maya(object):
    ...
    def get_json(self,**kwargs):
        ...
        if kwargs["task_id"]:
            ...
            sys.stdout.flush()
            print "org storage path is: " + options["platform"]["home_path"]
            sys.stdout.flush()
            options["platform"]["home_path"] = r"//www.storage.com/vcfs/input/d/inputdata5"
            print "NEW storage path is: " + options["platform"]["home_path"]
            sys.stdout.flush()
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] not in [1911075,1909920,1818014, 1844854, 1875231, 1846887, 1854355, 1814593, 963931, 1873017,
            ...
            cmd += " -preRender \"python \\\"pre_render_dict=%s;execfile(\\\\\\\"%s\\\\\\\")\\\"\";\"source \\\"//10.60.100.101/o5/py/model/user/1881915/premel_script/prerender2.mel\\\"\"" % (pre_render_dict,PreRender)
