...
def main(*args):
    ...
    for i in pm.ls(type="aiOptions"):
        ...
        if i.hasAttr("textureMaxMemoryMB"):
            ...
            FINAL_TMM = i.textureMaxMemoryMB.get()
            print "FINAL_TMM(textureMaxMemoryMB): %s " % (FINAL_TMM)
        if info_dict["task_id"] in [130013772,130013722,10457265,10461803,10512806]:
    ...
    for i in pm.ls(type="VRaySettingsNode"):
        if i.hasAttr("srdml"):
            ...
            if "vrayformaya" in info_dict["plugins"]:
                ...
                if info_dict["plugins"]["vrayformaya"] == "HQ" or info_dict["plugins"]["vrayformaya"] >= "3.10":
